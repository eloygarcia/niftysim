// =========================================================================
// File:       tledSolverGPU_kernels.cu
// Purpose:    CUDA kernels for tledSolverGPU class
...
#ifndef tledSolverGPU_kernels_CU
#define tledSolverGPU_kernels_CU
...
namespace tledSolverGPU_kernels
{
  __device__ float4 GetCurrentDisplacement(const int nodeIndex) {
    return tex1Dfetch(txUcurr, nodeIndex);
  }
...
if (tid < c_NumEls)
{
   ...
   g_Cb[tid] = make_float4(Cb.x,Cb.y,Cb.z,0.0f);
	...
	{
      tledSolverGPU_kernels::SPK_LE(tid,&Ca,&Cb,&SPKa,&SPKb);
	}
	else if (Vol_MType_K.y == 2)	// NH
	{
      float J = XT00*(XT11*XT22 - XT21*XT12) + XT01*(XT12*XT20 - XT10*XT22) + XT02*(XT10*XT21 - XT11*XT20);
      tledSolverGPU_kernels::SPK_NH(tid,&Ca,&Cb,J,&SPKa,&SPKb);
	}
	else if (Vol_MType_K.y == 3)	// TI
	{
		float J = XT00*(XT11*XT22 - XT21*XT12) + XT01*(XT12*XT20 - XT10*XT22) + XT02*(XT10*XT21 - XT11*XT20);
      tledSolverGPU_kernels::SPK_TI(tid,&Ca,&Cb,J,&SPKa,&SPKb);
	}
	else if (Vol_MType_K.y == 4)	// NHV
	{
		float J = XT00*(XT11*XT22 - XT21*XT12) + XT01*(XT12*XT20 - XT10*XT22) + XT02*(XT10*XT21 - XT11*XT20);
      tledSolverGPU_kernels::SPK_NHV(tid,&Ca,&Cb,J,g_StressStateIso,g_StressStateVol,&SPKa,&SPKb);
	}
	else if (Vol_MType_K.y == 5)	// TIV
	{
		float J = XT00*(XT11*XT22 - XT21*XT12) + XT01*(XT12*XT20 - XT10*XT22) + XT02*(XT10*XT21 - XT11*XT20);
      tledSolverGPU_kernels::SPK_TIV(tid,&Ca,&Cb,J,g_StressStateIso,g_StressStateVol,&SPKa,&SPKb);
	}
   else if (Vol_MType_K.y == 6)	// AB
	{
      float J = XT00*(XT11*XT22 - XT21*XT12) + XT01*(XT12*XT20 - XT10*XT22) + XT02*(XT10*XT21 - XT11*XT20);
      tledSolverGPU_kernels::SPK_AB(tid,&Ca,&Cb,J,&SPKa,&SPKb);
	}
   else if (Vol_MType_K.y == 7)	// PY
	{
      float J = XT00*(XT11*XT22 - XT21*XT12) + XT01*(XT12*XT20 - XT10*XT22) + XT02*(XT10*XT21 - XT11*XT20);
      tledSolverGPU_kernels::SPK_PY(tid,&Ca,&Cb,J,&SPKa,&SPKb);
	}
...
template <class TSolutionEvolutionData>
__global__ void ComputeNewDisps_kernel(float4* g_F, TSolutionEvolutionData* g_sols, tledGPUContacts* g_Contacts, bool* g_Divergence)
{
   const unsigned int tid = blockIdx.x*blockDim.x + threadIdx.x;
if (tid < c_NumNodes)
{
   ...
   g_F[tid] = tempF4;
   float4 R = tex1Dfetch(txF4Fext,tid);

   Unext = tledTimeStepper_kernels::ComputeNodalDisplacement<TSolutionEvolutionData>(g_sols, tid, make_float3(R.x,R.y,R.z) - make_float3(tempF4.x,tempF4.y,tempF4.z));
   // Get constraints
   int4 Mask = tex1Dfetch(txFixMask,tid);
   Unext = Unext*make_float3(Mask.x,Mask.y,Mask.z);
   tempF4 = tex1Dfetch(txUload,tid);
   Mask = tex1Dfetch(txDispMask,tid);
   if (Mask.x == 1)
      Unext.x = tempF4.x;
   if (Mask.y == 1)
      Unext.y = tempF4.y;
   ...
   if (c_numContactCyls > 0)
   {
      ...
      for (int i = 0; i < c_numContactCyls; i++)
      {
         ...
         if (isSlv > 0) // Check if current node is a slave node
         {
            ...
            tledSolverGPU_kernels::ResolveCylinderContact(&p,&cyl_orig,&cyl_axis,&Unext);
         }
      }
   }
   
   if (c_numContactPrbs > 0)
   {
      float4 p = tex1Dfetch(txNodeCds,tid) + make_float4(Unext.x,Unext.y,Unext.z,0.0f);
      for (int i = 0; i < c_numContactPrbs; i++)
      {
         int isSlv = g_Contacts->Prbs[i]->SlaveMask[tid];
         if (isSlv > 0) // Check if current node is a slave node
         {
            // NB: probe params arranged as
            // orig = (prb_orig.x,prb_orig.y,prb_orig.z)
            ...
            // Check penetration
            float4 prb_orig = g_Contacts->Prbs[i]->Origin;
            float4 prb_axis = g_Contacts->Prbs[i]->Axis;
            tledSolverGPU_kernels::ResolveUSProbeContact(&p,&prb_orig,&prb_axis,&Unext);
         }
      }
   }
   
   if (c_numContactPlts > 0)
   {
      float4 p = tex1Dfetch(txNodeCds,tid) + make_float4(Unext.x,Unext.y,Unext.z,0.0f);
      for (int i = 0; i < c_numContactPlts; i++)
      {
         int isSlv = g_Contacts->Plts[i]->SlaveMask[tid];
         if (isSlv > 0) // Check if current node is a slave node
         {
            // Check penetration
            float4 plt_a = g_Contacts->Plts[i]->a;
            float4 plt_b = g_Contacts->Plts[i]->b;
            float4 plt_c = g_Contacts->Plts[i]->c;
            tledSolverGPU_kernels::ResolvePlateContact(&p,&plt_a,&plt_b,&plt_c,&Unext);
         }
      }
   }
   ...
   g_sols->NextDisplacements[tid] = make_float4(Unext.x, Unext.y, Unext.z, 0.0f);
} // tid < NumNodes
}
...
#endif // tledSolverGPU_kernels_CU
